...
func (db *Database) ReplicateTo(target *Database, continuously bool) (*Replication, error) {
	...
	_, err := Do(db.server.httpClient, db.replicationURL(), "POST", db.Cred(), req, &resp)
	if err != nil {
		return nil, err
	}
...
func (repl *Replication) Cancel() error {
	...
	_, err := Do(repl.source.server.httpClient, repl.Source().replicationURL(), "POST", repl.source.Cred(), req, nil)
	return err
}
