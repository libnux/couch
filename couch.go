...
const (
	OPTIONS_INCLUDE_DOCS = "include_docs"
)
...
func NewServer(url string, cred *Credentials, httpClient *http.Client) *Server {
	if httpClient != nil {
		return &Server{url: url, cred: cred, httpClient: httpClient}

	} else {
		return &Server{url: url, cred: cred, httpClient: http.DefaultClient}
	}
}
...
func (s *Server) ActiveTasks() ([]Task, error) {
	var tasks []Task
	_, err := Do(s.httpClient, s.URL()+"/_active_tasks", "GET", s.Cred(), nil, &tasks)
	return tasks, err
}
...
func (db *Database) Create() error {
	_, err := Do(db.server.httpClient, db.URL(), "PUT", db.Cred(), nil, nil)
	return err
}

// DropDatabase deletes a database.
func (db *Database) DropDatabase() error {
	_, err := Do(db.server.httpClient, db.URL(), "DELETE", db.Cred(), nil, nil)
	return err
}
...
func (db *Database) Exists() bool {
	...
	req, err := http.NewRequest("HEAD", db.URL(), nil)
	if err != nil {
		return false
	}
	cred := db.Cred()
...
func (db *Database) Insert(doc Identifiable) error {
	...
	id, _ := doc.IDRev()
	if id == "" {
		_, err = Do(db.server.httpClient, db.URL(), "POST", db.Cred(), doc, &result)
	} else {
		_, err = Do(db.server.httpClient, db.docURL(id), "PUT", db.Cred(), doc, &result)
	}
	if err != nil {
		return err
	}
...
func (db *Database) retrieve(id, revID string, doc interface{}, options map[string]interface{}) error {
	...
	_, err := Do(db.server.httpClient, url, "GET", db.Cred(), nil, &doc)
	return err
}
...
func (db *Database) InsertBulk(bulk *Bulk, allOrNothing bool) (*Bulk, error) {
	...
	_, err := Do(db.server.httpClient, db.URL()+"/_bulk_docs", "POST", db.Cred(), bulk, &results)
...
func Do(httpClient *http.Client, url, method string, cred *Credentials, body, response interface{}) (*http.Response, error) {
